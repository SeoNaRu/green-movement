...
1. 전체 잔디 에너지에 따라 UFO가 필드에 양 1·2·4·6마리를 배치합니다.
2. 양은 도착한 잔디를 실제 물기 동작과 동기화해 먹고 포만도를 채웁니다.
3. 양 한 마리가 배부르면 UFO가 실제 위치에서 회수합니다. 패널은 즉시 다음 번호의 배고픈 양을 `INBOUND · 0/20`으로 예고하고, UFO는 이동 중간에는 보이지 않는 초고속 점멸 뒤 목적지 광원을 번쩍입니다. 새 양은 비행에 실리지 않고 그 목적지 칸에서만 나타납니다.
4. 하단 패널은 잔디 셀 그리드와 갈색 울타리선을 이어 받아 현재 양의 실제 `ENERGY n/20`, 물기 순간에 반응하는 10칸 포만도, 수거된 양, 전체 양 떼와 잔디 진행률을 보여 줍니다.
5. 마지막에도 먼저 먹은 양부터 UFO가 바로 수거하고, 모두 떠난 뒤 중앙 파동으로 GitHub 아이디를 남깁니다.

## 주요 특징

- 잔디 에너지에 따라 필드 인원을 1·2·4·6마리로 조절하고, 포만 즉시 원격 교대
- 첫 UFO는 화면 밖에서 미끄러져 들어오지 않고 첫 목적지에서 바로 점멸하며, 이후 모든 초기 투입도 중간 이동 없이 0.052초 점멸로 통일
- 교대 UFO는 0.104초 동안 중간 본체를 숨기고 목적지의 기존 광원으로 재등장
- 수거 직후 패널에서 다음 양의 새 번호와 `INBOUND · 0/20`을 먼저 보여 주고, 깨끗한 흰색·빈 포만도의 새 양을 목적지 칸에서만 생성
- 수거·투입 지점을 화면상 UFO 폭보다 멀리 분리하고, 새 양은 그 자리에서 착지해 잠깐 UFO와 함께 멈춘 뒤 기존 한 칸 속도로 이동
- 최대 마리 수 제한 없이 모든 양을 GitHub 잔디 톤의 두 줄 패널에 표시
- 선택 양의 실제 섭취량을 `ENERGY n/20`으로 표시하고, 10칸 포만도가 1단계 잔디는 반 칸·4단계 잔디는 두 칸씩 물기 순간에만 증가하며 짧게 반응
