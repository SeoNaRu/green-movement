...
3. 양 한 마리가 배부르면 UFO가 실제 위치에서 회수합니다. 다음 양의 로스터 칸이 약 0.7초 먼저 밝아지고, 배부른 양이 사라진 뒤 왼쪽 선택 영역을 0.104초 비운 다음 새 번호·빈 포만도의 양을 보여 줍니다. UFO는 초록빛으로 점멸해 다른 위치로 이동하고, 새 양은 그 목적지 칸에서만 나타납니다.
4. 하단 패널은 잔디 그리드와 같은 픽셀 셀, 기존의 절제된 갈색 외곽선으로 필드와 이어집니다. `방목·양떼·잔디`, 현재 양의 `포만 n/20`, 10칸 포만도, 모든 양의 상태를 한 화면에 표시합니다.
...
- 필드 양·큰 초상화·로스터 칸에 같은 고유 3×3 목장 귀표를 유지하고, 다음 양 칸을 미리 밝혀 빠른 교대에서도 서로 다른 개체임을 표시
...
- `방목·양떼·잔디` 상태 셀과 두 줄 로스터를 GitHub 잔디 셀 문법으로 통일하고, 완료 상태는 V형 체크 아이콘 없이 셀 채움과 문구로만 구분
