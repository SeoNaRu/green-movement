...
3. 양 한 마리가 배부르면 UFO가 실제 위치에서 회수합니다. 다음 양의 정사각 로스터 칸이 약 0.7초 먼저 밝아지고, 배부른 양이 사라진 뒤 왼쪽 선택 영역을 0.104초 비운 다음 새 번호·색상 귀표·빈 포만도의 양을 보여 줍니다. UFO는 초록빛으로 점멸해 다른 위치로 이동하고, 새 양은 그 목적지 칸에서만 나타납니다.
4. 하단 패널은 잔디 그리드와 같은 픽셀 셀, 기존의 절제된 갈색 외곽선으로 필드와 이어집니다. 전체 폭의 `방목·양떼·잔디`, 왼쪽 양·색상 귀표·번호·`포만 n/20`, 그리고 양 끝에 맞춰 균등 배치한 두 줄 로스터를 한 화면에 표시합니다.
...
- 필드 양·큰 초상화·로스터 칸에 같은 1×1 목장 귀표를 유지하고, 네 단계 기여 색상과 번호로 개체를 구분하며 다음 양 칸을 미리 표시
...
- 선택 양의 실제 섭취량을 `포만 n/20`으로 표시하고, 10칸 포만도가 1단계 잔디는 반 칸·4단계 잔디는 두 칸씩 물기 순간에만 증가하며 짧게 반응
- `방목·양떼·잔디` 상태 셀은 패널 전체 폭을 3등분하고, 포만도는 선택 양에만 표시하며, 정사각 두 줄 로스터는 available width에 `space-between`으로 배치
