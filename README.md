...
3. 양 한 마리가 배부르면 UFO가 실제 위치에서 회수한 뒤 다른 작업 구역으로 비행해, 번호가 다른 배고픈 양을 최소 3칸 떨어진 곳에 내려 교대시킵니다.
4. 하단 패널은 잔디 셀 그리드와 갈색 울타리선을 이어 받아 현재 양, 10칸 포만도, 수거된 양, 전체 양 떼와 잔디 진행률을 보여 줍니다.
5. 마지막에도 먼저 먹은 양부터 UFO가 바로 수거하고, 모두 떠난 뒤 중앙 파동으로 GitHub 아이디를 남깁니다.

## 주요 특징

- 잔디 에너지에 따라 필드 인원을 1·2·4·6마리로 조절하고, 포만 즉시 원격 교대
- 수거 위치와 다음 투입 위치를 분리하고, 새 양은 착륙 즉시 다음 잔디로 이동
- 최대 마리 수 제한 없이 모든 양을 GitHub 잔디 톤의 두 줄 패널에 표시
- 20점 포만도를 10개 잔디 셀로 표시해 1단계 잔디는 반 칸, 4단계 잔디는 두 칸씩 물기 순간에만 증가
