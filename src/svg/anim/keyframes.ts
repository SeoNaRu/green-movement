...
export function buildUfoLayer(params: {
  ...
  if (funnelPositionsEarly.length > 0) {
    ...
    ufoMoveKeyframePcts.push(
      `${exitStartPct.toFixed(4)}% { transform: translate(${lastTx}px, ${lastTy}px); }`,
    );
    ufoRotKeyframePcts.push(
      `${exitStartPct.toFixed(4)}% { transform: rotate(${lastSegAngle}deg); }`,
    );
    ufoMoveKeyframePcts.push(
      `100% { transform: translate(${lastTx}px, ${entryY}px); }`,
    );
  ...
  return { ufoKeyframesStr, ufoLightKeyframesStr, ufoGroupStr };
}
